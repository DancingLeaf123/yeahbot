...
local push_w_NE = function()
  return menu.farm_setting.lane_clear.push_w_NE:get() and not common.enemy_inrange() or
    not menu.farm_setting.lane_clear.push_w_NE:get()
end
  ...
  if get_spell_state() then
    ...
    if obj and menu.farm_setting.farm:get() then
      if obj.team == TEAM_ENEMY and menu.farm_setting.lane_clear.push_w:get() then
        if push_w_NE() then
          if
            push.get_minion_count_inrange(player.pos, get_total_radius()) >=
              menu.farm_setting.lane_clear.push_w_count.value
           then
            res = {obj = obj}
            return res
          end
      ...
      if (obj.team == TEAM_NEUTRAL and menu.farm_setting.jungle_clear.push_w:get()) then
        if
          push.get_minion_count_inrange(player.pos, get_total_radius()) >=
            menu.farm_setting.jungle_clear.push_w_count.value or (obj.highValue)
         then
          res = {obj = obj}
          return res
        end
